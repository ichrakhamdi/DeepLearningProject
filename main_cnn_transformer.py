...
class CNNTransformerExperimentRunner:
    def __init__(self, train_data, test_data):
        self.config = CNNTransformerSettings() # Use CNNTransformerSettings
        self.results = []
        self.train_df = train_data
        self.test_df = test_data
        # Variables to track the best model based on validation loss
        self.best_val_loss = float('inf')
        self.best_model_exp_id = None
        self.best_model_path = None # Path to the best checkpoint file
    ...
    def _run_single_experiment(self, class_type, aug, cw):
        # Create a new preprocessor for this specific experiment run
        preprocessor = DataPreprocessor()

        exp_id = f"cnn_transformer_{class_type}_aug{aug}_cw{cw}_win{self.config.WINDOW_SIZE}"
        print(f"\n=== Running CNN+Transformer experiment: {exp_id} ===")
        ...
        try:
            ...
            print(f"[Preprocessing Train] Shape after scaling/encoding (aug={aug}): X={X_train_processed.shape}, y={y_train_processed.shape}")
            ...
            # 5) Calculate num_classes and class_weights
            ...
            # 6) Build & train model using CNNTransformerTrainer
            trainer = CNNTransformerTrainer(experiment_id=exp_id)
            ...
            try:
                ...
                if current_min_val_loss < self.best_val_loss:
                    ...
                    self.best_model_path = self.config.MODELS_PATH / f'best_cnn_transformer_model_{exp_id}.h5'
            ...
            # 8) Save artifacts (scaler, encoder for this run) and plots
    ...
    def _save_run_artifacts(self, exp_id, scaler, encoder):
        # Save scaler and encoder specific to this run
    ...
    def _save_overall_best_model(self):
        """Copies the best model checkpoint and its corresponding artifacts to a standard name."""
        if self.best_model_exp_id and self.best_model_path:
            ...
            # Note the adjusted filenames for CNN+Transformer
            overall_best_model_dest = self.config.ARTIFACTS_PATH / "overall_best_cnn_transformer_model.h5"
            overall_best_scaler_dest = self.config.ARTIFACTS_PATH / "overall_best_cnn_transformer_scaler.joblib"
            overall_best_encoder_dest = self.config.ARTIFACTS_PATH / "overall_best_cnn_transformer_encoder.joblib"
            ...
            try:
                ...
                # Copy best encoder if it exists
    ...
    def _save_results(self):
        ...
        output_file = self.config.RESULTS_PATH / 'cnn_transformer_experiment_summary.csv' # Adjusted filename
        results_df.to_csv(output_file, index=False)
        print(f"\n=== All CNN+Transformer experiments finished. Results saved to: {output_file} ===")

if __name__ == "__main__":
    print("Starting CNN+Transformer Experiment Runner...")
