...
class TransformerExperimentRunner:
    ...
    def _run_single_experiment(self, class_type, aug, cw):
        exp_id = f"transformer_{class_type}_aug{aug}_cw{cw}"
    ...
    def _evaluate_and_save(self, exp_id, model, X_test, y_test, encoder):
        # Batch-wise evaluation to avoid CUDA invalid configuration errors
        model.eval()
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                if model.fc2.out_features == 1:  # Binary classification
        ...
        if model.fc2.out_features == 1:
            class_names = ['Benign', 'Attack']
        else:
            class_names = encoder.classes_ if encoder is not None else [
                str(i) for i in range(model.fc2.out_features)
            ]

        # Classification report & confusion matrix
        # Ensure labels align with target_names (handle missing classes)
    ...
    def _save_artifacts(self, exp_id, model, encoder):
        ...
        torch.save(model.state_dict(), self.config.MODELS_PATH / f"{exp_id}_model.pt")
    ...
    def _save_results(self):
        pd.DataFrame(self.results).to_csv(self.config.RESULTS_PATH / 'transformer_experiment_results.csv', index=False)

if __name__ == "__main__":
    runner = TransformerExperimentRunner(train_df, test_df)
    runner.run_experiments()
    print("=== All transformer experiments completed successfully ===") 